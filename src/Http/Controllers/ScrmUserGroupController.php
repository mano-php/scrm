 ...
 * 客户系统分群
 *
 * @property ScrmUserGroupService $service
 */
class ScrmUserGroupController extends AdminController
{
	protected string $serviceName = ScrmUserGroupService::class;
...
//				...$this->baseHeaderToolBar()
			...
			->columns([
                ...
                amis()->SelectControl('up_type', '更新频率')->options(ScrmUserGroupService::UP_TYPE)->static(),
...
//		]);
        ...
        return amis()->Service()->body([
            amis()->CRUDTable()->source('$rows')->columns([
                amis()->TableColumn('id', 'ID')->sortable(),
                amis()->TableColumn('name', '人群名称'),
                amis()->TableColumn('up_type', '更新频率')->options(ScrmUserGroupService::UP_TYPE)->static(),
            ])
        ])->api(admin_url('/scrm_user').'?_action=getData');
